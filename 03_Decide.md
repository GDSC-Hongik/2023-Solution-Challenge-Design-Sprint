...
## 아이디어 4. 다른 부가 서비스
기존 지도앱의 경우 화장실 등의 장소는 잘 표시되지 않았는데, 이런 정보를 표시하는 것도 좋을 것 같다.
- 특히 화장실의 청결도가 중요한 사람들이 있다. 이 경우 화장실의 청결도를 나타내는 정보를 담으면 좋을 것
- 또한 아이의 기저귀 교환대가 설치되어 있는 화장실과 그렇지 않은 화장실이 존재한다. 특히 여자 화장실이 아닌 남자 화장실의 경우 기저귀 교환대가 없는 시설이 대다수인데, 이것이 육아를 하는 남편에게 있어서 불편한 요소이다. 아이 기저귀 등의 육아 등에 꼭 필요하지만, 잘 드러나지 않는 정보들을 담는 서비스를 제공하자.

### 장점
- 사람들이 잘 사용하진 않지만, 어느 결정적인 순간에 꼭 필요한 정보를 즉각적으로 받아볼 수 있다.

### 단점
- 화장실의 청결도 정보 등은 기본적으로 사람들이 직접 평가를 해야 하는 참여 형태의 조사이므로 조사가 되지 않는 부분이 더 많을 수 있다.
- 또한 기저귀 교환대 등의 세부 정보의 경우 자동적으로 정보를 추출하기가 어렵다. 마땅한 API가 없어 기술적 구현 난이도가 높을 것으로 예상된다.
...
## 각 아이디어에 대한 기본 디자인
아래의 예시들은 기본적인 예시임. 추후 변경 가능

1. 아이디어 1. 공원 추천 지도
<img src="./img/slide1.JPG"/>
공원, 혹은 청년 쉼터 등을 표시하는 형식
예를 들어 우측 상단의 노란색 버튼을 누르면, 화살표로 공원이 표시되고, 해당 화살표를 누르면 공원에 대한 정보가 팝업처럼 나오는 형식을 생각함.

2. 아이디어 2. 이벤트 밀도 지도
<img src="./img/slide2.JPG"/>
이벤트 밀도를 색깔 별로 해서 덮는 형식.
예를 들어 우측 상단의 주황색 버튼을 누르면, 지도에서 이벤트 밀도가 색으로 표시됨.

3. 아이디어 3. 공간 기반 커뮤니티
<img src="./img/slide3.JPG"/>
게시물이 없다면 그냥 지도처럼 평범하게 뜨다가, 만약 게시물이 하나 이상 있을 경우 그 지역 범위 내에 어떠한 표시가 뜸. 그 표시를 클릭하면 팝업처럼 그 지역에서 작성된, 혹은 그 지역을 태그한 게시물들이 올라오는 형식.